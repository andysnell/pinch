...
final class Rs256ProtocolRsaTest extends TestCase
{
    ...
    #[Test]
    public function signAndVerifyRoundTripWithRsaKeys(): void
    {
        // Generate proper RSA key pair for RS256
    ...
    #[Test]
    public function verifyRejectsInvalidSignatureWithRsaKeys(): void
    {
        ...
        // Sign with first RSA key
        $jwt = $this->protocol->sign($keyPair1, $header, $payload);

        // Try to verify with different RSA key - should fail
    ...
    #[Test]
    public function supportsMultipleRsaKeySizes(): void
    {
        // Test only secure key sizes (2048-bit keys removed for security)
        foreach ([3072, 4096] as $keySize) {
            $keyPair = RsaSignatureKeyPair::generate($keySize);

            $header = new JwtHeader(JwtAlgorithm::RS256);
            $payload = new JwtPayload(['key_size' => $keySize], $this->clock);

            $jwt = $this->protocol->sign($keyPair, $header, $payload);
            $decoded = $this->protocol->verify($keyPair->publicKey(), $jwt);

            self::assertSame($keySize, $decoded->payload->claims['key_size']);
            self::assertSame($keySize, $keyPair->secret->keySize);
        }
    }
}
